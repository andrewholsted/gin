...
function generate(name){
  ...
  // Change 'Directive Name' to page-name
  fileName = changeCase.param(name);

  // Get the path of the current directory (should be project root) and map to destination path
  dir = path.join(process.cwd(), 'core/client/app/components', fileName);

  // Create the directory in core/client/app/components
...
function writeFile(err, content, template, callback){
  ...
  // Regex replace of the directive name
  if(template == 'directive-template.js') {
    content = content.replace('directiveName', changeCase.camelCase(fileName));
    content = content.replace(/directive-name/g, fileName)
  }

  fs.writeFile(newFile, content , function(err){
    if(err) {
      callback(err) // This callback is for the async.each if we hit an error.
    }
    else{
      callback()
    }
